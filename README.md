...
添加书源后可自动发现并采集该书源下的所有小说到书架，配置项在 `conf.yml` 中：

```yaml
auto:
  crawl: true    # 添加书源后自动采集全部书籍
  update: true   # 启用书籍章节定时更新（需 admin.update 同时为 true）
```

## 工作流程

1. **添加书源时自动触发**：通过 App 或后台导入书源后，服务端自动开始采集
2. **全量发现**：使用书源的 `ruleExplore`（发现规则）遍历所有分类/页面
3. **去重入库**：检查书籍是否已在书架，自动跳过重复
4. **章节更新**：采集后自动获取目录信息，后续通过定时任务保持最新
5. **断点续采**：采集任务中断后，每 10 分钟自动恢复

## API 接口

| 接口 | 方法 | 说明 |
|------|------|------|
| `/api/5/getCrawlTasks` | GET | 查看所有采集任务状态 |
| `/api/5/startCrawl` | GET | 手动触发指定书源采集 |
| `/api/5/delCrawlTask` | GET | 删除采集任务记录 |
